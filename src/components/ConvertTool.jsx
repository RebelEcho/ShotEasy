...
const unsupportedReason = '此文件类型在所选模式下不支持。';
...
export default function ConvertTool({ copy = {} }) {
    ...
    const addFiles = async (files) => {
        ...
        if (acceptedFiles.length !== sortedFiles.length) {
            messageApi.warning('不支持的文件已被跳过。');
        }
        if (!acceptedFiles.length) {
            messageApi.warning('请选择支持的图片或 PDF 文件。');
            return;
        }
        ...
        try {
            ...
            if (hasLoadError) messageApi.error('部分图片无法加载。');
        } catch (error) {
            cleanupItems(loadedItems);
            messageApi.error(error.message || '文件加载失败。');
        } finally {
            setLoading(false);
        }
    };
    ...
    const addFolder = async () => {
        ...
        try {
            ...
            if (error.name !== 'AbortError') messageApi.error(error.message || '文件夹选择失败。');
        }
    };
    ...
    const convertImagesToPdf = async (sourceItems, selectedMode) => {
        ...
        if (!compatibleItems.length) {
            if (compatibleCount) {
                messageApi.error('没有图片可转换为 PDF。');
            } else {
                messageApi.warning('请添加所选模式支持的图片。');
            }
        ...
        try {
            ...
            const fallback = await markPdfSerializationFailures(compatibleItems, error.message || 'PDF 序列化失败。');
            pdfItems = fallback.serializableItems;
            serializationFailedIds = fallback.failedIds;

            if (!pdfItems.length) {
                messageApi.error('没有图片可序列化为 PDF。');
            ...
            try {
                ...
                const errorMessage = retryError.message || 'PDF 转换失败。';
    ...
    const convertAll = async () => {
        ...
        try {
            const convertedItems = selectedMode.value === 'images-to-pdf'
                ...
                : await Promise.all(sourceItems.map(async (item) => {
                    try {
                        ...
                        return { ...item, status: 'error', error: error.message || '转换失败。', outputs: [] };
                    }
                }));
            ...
            setItems(sourceItems.map((item) => ({ ...item, status: 'error', error: error.message || '转换失败。', outputs: [] })));
