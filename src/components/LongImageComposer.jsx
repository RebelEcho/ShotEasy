...
const LongImageComposer = () => {
    ...
    const sizeLabel = option.direction === 'vertical' ? '输出宽度' : '输出高度';
    ...
    const addFiles = async (files) => {
        ...
        if (!inputFiles.length) {
            messageApi.warning('请选择图片或 PDF 文件。');
            return;
        }
        setLoading(true);
        try {
            ...
            messageApi.error(error.message || '图片加载失败。');
        } finally {
            setLoading(false);
        }
    };
    ...
    const compose = async () => {
        if (!layout || !items.length) {
            messageApi.warning('请先添加图片。');
            return;
        }
        if (layout.width > 32767 || layout.height > 32767) {
            messageApi.error('输出尺寸过大，超出浏览器画布限制。请减小输出尺寸。');
            return;
        }
        setLoading(true);
        try {
            ...
            messageApi.success('合成成功！');
        } catch (error) {
            messageApi.error(error.message || '合成失败。');
        } finally {
            setLoading(false);
        }
    };
        ...
        listComponent = (
            <div className="w-full bg-white shadow-md rounded-md overflow-hidden border-t-gray-600">
                <div className="bg-gray-800 py-1 px-3 text-white text-sm flex flex-wrap items-center gap-x-5 gap-y-1">
                    ...
                    <span className="text-slate-400">图片:<span className="text-white ml-1">{items.length}</span></span>
                    <span className="text-slate-400">预估尺寸:<span className="text-green-400 ml-1 font-semibold">{estimatedSize}</span></span>
                </div>
                    ...
                    {items.map((item, index) => (
                        <div key={item.id} className="flex items-center justify-between pb-5 relative after:block after:absolute after:bottom-0 after:left-16 after:right-0 after:h-[1px] after:bg-slate-200">
                            <div className="flex items-center min-w-0 max-w-[62%]">
                                <img src={item.src} alt={`${item.name || '图片'} 长图合成素材预览`} className="w-12 h-12 mr-4 rounded-md object-cover flex-shrink-0 bg-slate-100" />
            ...
            <div className="bg-white shadow-md rounded-md overflow-hidden border-t border-t-gray-600">
                <div className="flex gap-4 p-2 justify-center flex-col-reverse md:flex-row md:justify-between">
                    <div className="flex gap-3 items-center justify-center flex-wrap">
                        <Upload name="file" multiple={true} beforeUpload={beforeUpload} showUploadList={false} accept={inputTypes.join(',')}>
                            <Button disabled={loading} icon={<Icon name="ImagePlus" />}>添加图片 / PDF</Button>
                        </Upload>
                        {supportsDirectoryPicker && <Button disabled={loading} icon={<Icon name="FolderPlus" />} onClick={addFolder}>添加文件夹</Button>}
                    </div>
                    {items.length > 0 &&
                        <div className="flex gap-3 items-center justify-center">
                            <Tooltip placement="top" title="合成">
                                <Button
                                    type="primary"
                                    className="bg-black"
                                    loading={loading}
                            ...
                            <Tooltip placement="top" title={result ? "下载合成图片" : "请先合成"}>
                                <Button
                                    type="primary"
                                    className="bg-black"
                                    disabled={!result}
                            ...
                            <Tooltip placement="top" title="清空所有">
                ...
                <div className="py-1 px-2 flex items-center gap-4 border-t border-dotted bg-slate-50 text-xs select-none flex-wrap">
                    <div className="flex gap-2 items-center">
                        <label className="font-semibold">方向:</label>
                        <Radio.Group
                            size="small"
                ...
                <div className="relative min-h-[240px] p-5" style={dotBackground}>
                    ...
                    {result && (
                        <div className="mt-4 mx-auto max-w-3xl rounded-md bg-white shadow-md p-4">
                            ...
                            {result.format === 'pdf'
                                ? <iframe title="PDF 预览" src={result.url} className="w-full h-[520px] rounded border" />
                                : <img src={result.url} alt="合成后的长图预览" className="block max-w-full max-h-[620px] mx-auto rounded border object-contain" />
                            }
                        </div>
                    )}
                </div>
