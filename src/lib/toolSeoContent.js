...
const TOOL_FEATURES = {
    ...
    compressor: ['批量压缩', 'WebAssembly 编码', '支持 JPG、PNG、WebP、GIF、SVG、AVIF'],
    convert: ['图片格式转换', 'PDF 导出', '本地 Canvas 处理'],
    rounded: ['圆角裁剪', '透明背景输出', '头像和产品图样式'],
    remover: ['AI 背景去除', '透明 PNG 输出', '本地模型处理'],
    blurBackground: ['AI 主体识别', '可调背景虚化', '本地 Canvas 合成'],
    longImage: ['图片垂直拼接', '间距和宽度控制', '长图导出'],
    viewer: ['DOCX、PPTX、XLSX、CSV、PDF、压缩包预览', '无需安装 Office 套件', '浏览器文件查看'],
    videoConvert: ['视频转 GIF', '格式转换', '压缩、裁剪、剪切、提取音频']
};

export const getToolSeoContent = (locale, tool, t = {}) => {
    const copy = LOCALE_COPY[locale] || LOCALE_COPY['zh-CN'];
