...
export const getViewerAliasMeta = (viewer, alias, locale = 'zh-CN') => {
  if (!viewerAliases.includes(alias)) return viewer;

  const normalizedLocale = routeTerms[locale] ? locale : 'zh-CN';
  const term = routeTerms[normalizedLocale][alias] || routeTerms['zh-CN'][alias];
  const benefit = titleBenefits[normalizedLocale] || titleBenefits['zh-CN'];
  const intro = descriptionIntros[normalizedLocale] || descriptionIntros['zh-CN'];
  const subtitle = routeSubtitles[normalizedLocale]?.[alias] || routeSubtitles['zh-CN'][alias];
